...
class LoanedBookInstancesByUseListViewTest(TestCase):
    def setUp(self):
        # create two users
        ...
        # direct assigned which is nt allowed
    ...
    def test_logged_in_uses_correct_template(self):
        ...
        self.assertEqual(response.status_code, 200)
        # check we got the correct template
    ...
    def test_only_borrowed_books_in_list(self):
        ...
        # check our user is logged in
        self.assertEqual(str(response.context['user']), 'testuser1')
        # check that we gota response "success"
        self.assertEqual(response.status_code, 200)

        # check that initially we don't have any books in list
...
class RenewBookInstancesViewTest(TestCase):
    ...
    def test_form_renewal_date_initially_has_date_three_weeks_in_future(self):
        ...
        self.assertEqual(response.status_code, 200)
        date_far_in_future = datetime.date.today() + datetime.timedelta(weeks=3)
        self.assertEqual(
            response.context['form'].initial['due_back'], date_far_in_future)
    ...
    def test_form_invalid_date_future(self):
        login = self.client.login(
            username='testuser2', password='2HJ1vRV0Z&3iD')
        ...
        self.assertEqual(response.status_code, 200)
        self.assertFormError(
            response.context['form'], 'due_back', 'Invalid date - renewal more than 4 weeks ahead')
...
class AuthorCreateViewTest(TestCase):
    ...
    def test_user_with_permission_to_create_author(self):
        login = self.client.login(
            username='testuser2', password='2HJ1vRV0Z&3iD')
        response = self.client.post(reverse('author-create'), {
            'first_name': "John",
            'last_name': "Smith",
            'date_of_birth': '11/11/1923'
        })
        self.assertEqual(response.status_code, 302)
        # verify that was successfully created
        self.assertEqual(Author.objects.count(), 1)

        author = Author.objects.first()
        response = self.client.get(reverse('author-detail', kwargs={
            'pk': author.id
        }))
        self.assertEqual(response.context.get('object').first_name, "John")

    def test_user_without_permission_to_create_author(self):
        login = self.client.login(
            username='testuser1', password='1X<ISRUkw+tuK')
        response = self.client.post(reverse('author-create'), {
            'first_name': "John",
    ...
    def test_redirects_to_detail_view(self):
        login = self.client.login(
            username='testuser2', password='2HJ1vRV0Z&3iD')
        response = self.client.post(reverse('author-create'), {
            'first_name': "John",
